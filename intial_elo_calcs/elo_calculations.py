    ...
    for driver in drivers:
        for driver_construcor, elo in driver.constructor_to_elo.items():  # Get both elo and constructor name from dict
            if driver_construcor.constructor_name == constructor.constructor_name:  # Ensure we are only doing it for the constructor we want
                ...
                count += 1  # account for changes (instead of assumig 20)
...
df = pd.read_csv("../preprocessing/final_data.csv")
...
for _, row in df.iterrows():
    ...
    #name data
    driver_name = row["racer_name"]
    constructor_name = row["constructor_name"]

    #data needed for elo
    pole_points = row["finish_points"]
    qual_points = row["qual_points"]
    #teammate_points = row["teammate_points"]

#print(df[["finish_points", "qual_points", "teammate_points"]].isna().sum())

df["elo"] = df.apply(lambda x: elo_calculation(pole_points, qual_points), axis=1)
grouped = df.groupby(["year", "racer_name", "constructor_name"])["elo"].sum().reset_index()
print(grouped)
